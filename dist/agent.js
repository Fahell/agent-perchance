var v="";function h(e){v=e}function A(){return v}async function b(e){try{return(await fetch("https://s.jina.ai/",{method:"POST",headers:{Authorization:`Bearer ${e}`,"Content-Type":"application/json",Accept:"application/json"},body:JSON.stringify({q:"test"})})).ok}catch{return!1}}function S(e){return{Authorization:`Bearer ${v}`,"Content-Type":"application/json",Accept:"application/json",...e}}async function E(e,t=5){let r=await fetch("https://s.jina.ai/",{method:"POST",headers:S({"X-Return-Format":"json"}),body:JSON.stringify({q:e,num:t})});if(!r.ok)throw new Error(`Jina search failed: ${r.status} ${r.statusText}`);let n=await r.json(),o=(n.data||n||[]).slice(0,t).map(s=>({title:s.title||"",url:s.url||"",description:s.description||"",content:s.content||""})),a=o.map((s,u)=>`[${u+1}] ${s.title}
    ${s.url}
    ${s.description}`).join(`

`);return{query:e,results:o,raw:a}}async function R(e,t=5e3){let r=await fetch("https://r.jina.ai/",{method:"POST",headers:S({Accept:"text/markdown"}),body:JSON.stringify({url:e})});if(!r.ok)throw new Error(`Jina scrape failed: ${r.status} ${r.statusText}`);let n=await r.text(),o=n.match(/^#\s+(.+)/m),a=o?o[1]:new URL(e).hostname,s=n.length>t?n.slice(0,t)+`
...
${t.content}`}}};function I(e){return w[e]}function $(){return Object.values(w).map(e=>`- ${e.name}: ${e.description}
  Parameters: ${JSON.stringify(e.parameters)}`).join(`
`)}function M(e){return e in w}var D=5;var k=/<tool_call\s+name="(\w+)">\s*(\{.*?\})\s*<\/tool_call>/gs;function N(){return`You are a helpful assistant with access to web search.
...
Never make up data \u2014 if the search fails, tell the user.`}function F(e){let t=[],r;for(k.lastIndex=0;(r=k.exec(e))!==null;){let[,n,o]=r;try{let a=JSON.parse(o);M(n)&&t.push({name:n,args:a})}catch{console.warn(`[Agent] Failed to parse tool_call args: ${o}`)}}return t}function U(e){return e.replace(k,"").trim()}async function B(e,t,r=[],n,o){let s=N()+`

`;if(r.length>0){s+=`Recent conversation:
`;for(let g of r){let f=g.role==="user"?"User":"Assistant";s+=`${f}: ${g.content}
`}s+=`
`}s+=`User message: ${t}`;let u=0;for(;u<D;){u++,n?.(`Thinking... (step ${u})`);let f=(await e.generateText({instruction:s})).toString(),y=F(f);if(y.length===0)return U(f);for(let l of y){let x=I(l.name);if(x){n?.(`Using ${l.name}...`);try{let c=await x.execute(l.args);o?.(l.name,l.args,c);let d=`
...
The tool failed. Respond to the user explaining the issue.`}}}}return"I apologize, but I wasn't able to complete that task after multiple attempts."}var m=null;function O(e){m=e}function H(){return m?.thread?.customData?m.thread.customData:{}}function z(e){try{return H()[e]??void 0}catch(t){console.warn("[Storage] get("+e+") failed:",t);return}}function C(e,t){try{if(!m?.thread?.customData){console.warn("[Storage] set: oc.thread.customData not available");return}m.thread.customData[e]=t}catch(r){console.warn("[Storage] set("+e+") failed:",r)}}var i=window.oc,_=!1,W=10;function J(){console.log("\u{1F916} Agent v0.1.0+dd636c7"),console.log("   Build: 2026-06-27 23:12:16"),console.log("   https://github.com/Fahell/agent-perchance")}var P="agent:jina_key";function Y(){return z(P)??null}function L(e){C(P,e)}function q(){return i?i.thread?typeof i.generateText!="function"?(console.error("\u274C [Agent] oc.generateText not available"),!1):!0:(console.error("\u274C [Agent] oc.thread not available"),!1):(console.error("\u274C [Agent] window.oc not found \u2014 are you running inside Perchance?"),!1)}function V(){document.body.innerHTML=`
    <div style="font-family: system-ui; padding: 24px; background: #1a1a2e; color: #eee; height: 100vh; margin: 0; display: flex; align-items: center; justify-content: center;">
      <div style="max-width: 480px; width: 100%;">
        <div style="text-align: center; margin-bottom: 24px;">
          ...
          <span style="font-size: 11px; color: #666;">v0.1.0+dd636c7</span>
        </div>
  ...
  `,document.getElementById("settings-btn").addEventListener("click",G),i.window.show(),console.log("\u{1FA9F} [Agent] Window opened")}function G(){let e=A(),t=e?e.slice(0,8)+"..."+e.slice(-4):"Nenhuma",r=document.createElement("div");r.id="settings-overlay",r.style.cssText="position:fixed;top:0;left:0;right:0;bottom:0;background:rgba(0,0,0,0.7);z-index:1000;display:flex;align-items:center;justify-content:center;font-family:system-ui;",r.innerHTML=`
  ...
  `,document.body.appendChild(r),document.getElementById("settings-close").addEventListener("click",()=>r.remove()),document.getElementById("settings-save").addEventListener("click",async()=>{let n=document.getElementById("settings-key-input").value.trim(),o=document.getElementById("settings-msg");if(!n){o.textContent="Insira uma chave.",o.style.color="#f87171",o.style.display="block";return}o.textContent="Validando...",o.style.color="#aaa",o.style.display="block",await b(n)?(L(n),h(n),o.textContent="\u2705 Chave salva!",o.style.color="#4ade80",setTimeout(()=>r.remove(),1e3)):(o.textContent="\u274C Chave inv\xE1lida.",o.style.color="#f87171")})}function Z(e){let t=e.trim();return t.startsWith("/agent")||t.startsWith("/test-storage")}function Q(e){let t=e.trim();t==="/agent open"?(i.window.show(),console.log("\u{1FA9F} [Agent] Window opened")):t==="/agent close"?(i.window.hide(),console.log("\u{1FA9F} [Agent] Window closed")):t==="/test-storage"||t==="/test-storage run"?ee():t==="/test-storage check"?te():t==="/test-storage clean"&&(K(),p(`<div style="margin:8px 0;padding:8px;background:#16213e;border-radius:6px;border-left:3px solid #4ade80;">
  ...
  </div>`);let r=[{label:"1KB",bytes:1024},{label:"5KB",bytes:5*1024},{label:"10KB",bytes:10*1024},{label:"50KB",bytes:50*1024},{label:"100KB",bytes:100*1024},{label:"500KB",bytes:500*1024},{label:"1MB",bytes:1048576},{label:"5MB",bytes:5*1048576},{label:"10MB",bytes:10*1048576},{label:"25MB",bytes:25*1048576},{label:"50MB",bytes:50*1048576},{label:"100MB",bytes:100*1048576}];K();let n=[],o="";for(let{label:a,bytes:s}of r){p(`<div style="margin:2px 0 2px 12px;padding:4px 8px;color:#aaa;font-size:12px;">\u23F3 ${a} (${t(s)})...</div>`);let u="ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789",g="";for(;g.length<s;)g+=u;g=g.slice(0,s);let f=!1,y=0,l=!1,x=0;try{let d=performance.now();i.thread.customData[e+a]=g,y=Math.round(performance.now()-d),f=!0}catch(d){p(`<div style="margin:1px 0 1px 16px;padding:2px 8px;font-size:11px;color:#f87171;">
        \u274C WRITE FAILED: ${d instanceof Error?d.message:d}
      </div>`),n.push(`${a}\u274C`);break}try{let d=performance.now(),j=i.thread.customData[e+a];x=Math.round(performance.now()-d),l=j===g}catch{l=!1}let c=f&&l;if(p(`<div style="margin:1px 0 1px 16px;padding:2px 8px;font-size:11px;color:${c?"#4ade80":"#f87171"};">
      ${c?"\u2705":"\u274C"} Write: ${y}ms | Read: ${x}ms
  ...
  </div>`)}function te(){let e="__test_",t=["1KB","5KB","10KB","50KB","100KB","500KB","1MB","5MB","10MB","25MB","50MB","100MB"],r=i.thread.customData||{},n=0;p(`<div style="margin:8px 0;padding:8px;background:#1a1a2e;border-radius:6px;border-left:3px solid #f59e0b;">
  ...
  </div>`);let t=i.thread.messages.filter(n=>(n.author==="user"||n.author==="ai")&&n!==e).slice(-W).map(n=>({role:n.author==="user"?"user":"assistant",content:n.content})),r=await B(i,e.content,t,n=>{console.log("\u{1F916} [Agent]",n)},(n,o,a)=>{let s=o.query||o.url||"",u=a.slice(0,300).replace(/\n/g," ");p(`<div style="margin: 4px 0 4px 12px; padding: 6px; background: #0f3460; border-radius: 4px; border-left: 2px solid #4ade80;">
