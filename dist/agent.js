...
Never make up data \u2014 if the search fails, tell the user.`}function E(e){let t=[],n;for(p.lastIndex=0;(n=p.exec(e))!==null;){let[,r,i]=n;try{let a=JSON.parse(i);v(r)&&t.push({name:r,args:a})}catch{console.warn(`[Agent] Failed to parse tool_call args: ${i}`)}}return t}function S(e){return e.replace(p,"").trim()}async function R(e,t,n,r){let a=`${O()}

User message: ${t}`,o=0;for(;o<$;){o++,n?.(`Thinking... (step ${o})`);let h=(await e.generateText({instruction:a})).toString(),m=E(h);if(m.length===0)return S(h);for(let c of m){let w=T(c.name);if(w){n?.(`Using ${c.name}...`);try{let l=await w.execute(c.args);r?.(c.name,c.args,l);let u=`
...
The tool failed. Respond to the user explaining the issue.`}}}}return"I apologize, but I wasn't able to complete that task after multiple attempts."}var s=window.oc;function I(){console.log("\u{1F916} Agent v0.1.0+f11f556"),console.log("   Build: 2026-06-27 03:04:20"),console.log("   https://github.com/Fahell/agent-perchance")}function M(){return s?s.thread?typeof s.generateText!="function"?(console.error("\u274C [Agent] oc.generateText not available"),!1):!0:(console.error("\u274C [Agent] oc.thread not available"),!1):(console.error("\u274C [Agent] window.oc not found \u2014 are you running inside Perchance?"),!1)}function L(){document.body.innerHTML=`
    <div style="font-family: system-ui; padding: 16px; background: #1a1a2e; color: #eee; height: 100vh; margin: 0; display: flex; flex-direction: column;">
      <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 12px;">
        ...
        <span style="font-size: 11px; color: #666;">v0.1.0+f11f556 \xB7 2026-06-27 03:04:20</span>
      </div>
