...
`)}function v(e){return e in g}var R=5,f=/<tool_call\s+name="(\w+)">\s*(\{.*?\})\s*<\/tool_call>/gs;function $(){return`You have access to the following tools:
...
- For general knowledge questions, respond directly without using tools.`}function A(e){let t=[],n;for(f.lastIndex=0;(n=f.exec(e))!==null;){let[,o,r]=n;try{let s=JSON.parse(r);v(o)&&t.push({name:o,args:s})}catch{console.warn(`[Agent] Failed to parse tool_call args: ${r}`)}}return t}function O(e){return e.replace(f,"").trim()}async function b(e,t,n,o){let s=`${$()}

User message: ${t}`,l=0;for(;l<R;){l++,n?.(`Thinking... (step ${l})`);let u=(await e.generateText({instruction:s})).toString(),m=A(u);if(m.length===0)return O(u);for(let c of m){let x=_(c.name);if(x){n?.(`Using ${c.name}...`);try{let d=await x.execute(c.args);o?.(c.name,c.args,d);let p=`

[Tool Result - ${c.name}]:
${d}

Now respond to the user based on this information. Do NOT use any more tools \u2014 give your final answer.`;s+=p}catch(d){let p=d instanceof Error?d.message:String(d);console.error(`[Agent] Tool ${c.name} failed:`,p),s+=`

[Tool Error - ${c.name}]: ${p}

The tool failed. Respond to the user explaining the issue.`}}}}return"I apologize, but I wasn't able to complete that task after multiple attempts."}var i=window.oc;function E(){console.log(`%c\u{1F916} Agent %cv0.1.0+8821bdc%c
   Build: 2026-06-27 01:00:58
   https://github.com/Fahell/agent-perchance`,"color: #00d4ff; font-weight: bold; font-size: 14px;","color: #4ade80; font-weight: bold; font-size: 14px;","color: #888; font-size: 11px;")}function S(){return i?i.thread?typeof i.generateText!="function"?(console.error("\u274C [Agent] oc.generateText not available"),!1):!0:(console.error("\u274C [Agent] oc.thread not available"),!1):(console.error("\u274C [Agent] window.oc not found \u2014 are you running inside Perchance?"),!1)}function I(){document.body.innerHTML=`
