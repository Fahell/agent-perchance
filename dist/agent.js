var x="";function g(e){x=e}function T(){return x}async function m(e){try{return(await fetch("https://s.jina.ai/",{method:"POST",headers:{Authorization:`Bearer ${e}`,"Content-Type":"application/json",Accept:"application/json"},body:JSON.stringify({q:"test"})})).ok}catch{return!1}}function k(e){return{Authorization:`Bearer ${x}`,"Content-Type":"application/json",Accept:"application/json",...e}}async function E(e,t=5){let s=await fetch("https://s.jina.ai/",{method:"POST",headers:k({"X-Return-Format":"json"}),body:JSON.stringify({q:e,num:t})});if(!s.ok)throw new Error(`Jina search failed: ${s.status} ${s.statusText}`);let n=await s.json(),o=(n.data||n||[]).slice(0,t).map(r=>({title:r.title||"",url:r.url||"",description:r.description||"",content:r.content||""})),a=o.map((r,l)=>`[${l+1}] ${r.title}
    ${r.url}
    ${r.description}`).join(`

`);return{query:e,results:o,raw:a}}async function S(e,t=5e3){let s=await fetch("https://r.jina.ai/",{method:"POST",headers:k({Accept:"text/markdown"}),body:JSON.stringify({url:e})});if(!s.ok)throw new Error(`Jina scrape failed: ${s.status} ${s.statusText}`);let n=await s.text(),o=n.match(/^#\s+(.+)/m),a=o?o[1]:new URL(e).hostname,r=n.length>t?n.slice(0,t)+`

[...truncated]`:n;return{url:e,title:a,content:r}}var h={web_search:{name:"web_search",description:"Search the web for REAL-TIME or CURRENT information. USE this for: prices, exchange rates, sports results, news, weather, events, recent facts, or anything you are not 100% sure about. Returns up to 5 results with titles, URLs, and descriptions.",parameters:{query:"The search query string. Be specific \u2014 include topic, year, or context when relevant."},execute:async e=>(await E(e.query,5)).raw||"No results found."},scrape_url:{name:"scrape_url",description:"Fetch and extract the full text content from a specific URL as markdown. USE this when you have a URL and need to read its content.",parameters:{url:"The full URL to scrape (must start with http:// or https://)"},execute:async e=>{let t=await S(e.url,5e3);return`# ${t.title}

${t.content}`}}};function I(e){return h[e]}function R(){return Object.values(h).map(e=>`- ${e.name}: ${e.description}
  Parameters: ${JSON.stringify(e.parameters)}`).join(`
`)}function $(e){return e in h}var P=5;var v=/<tool_call\s+name="(\w+)">\s*(\{.*?\})\s*<\/tool_call>/gs;function z(){return`You are a helpful assistant with access to web search.
...
Never make up data \u2014 if the search fails, tell the user.`}function L(e){let t=[],s;for(v.lastIndex=0;(s=v.exec(e))!==null;){let[,n,o]=s;try{let a=JSON.parse(o);$(n)&&t.push({name:n,args:a})}catch{console.warn(`[Agent] Failed to parse tool_call args: ${o}`)}}return t}function j(e){return e.replace(v,"").trim()}async function O(e,t,s=[],n,o){let r=z()+`

`;if(s.length>0){r+=`Recent conversation:
`;for(let y of s){let p=y.role==="user"?"User":"Assistant";r+=`${p}: ${y.content}
`}r+=`
`}r+=`User message: ${t}`;let l=0;for(;l<P;){l++,n?.(`Thinking... (step ${l})`);let p=(await e.generateText({instruction:r})).toString(),A=L(p);if(A.length===0)return j(p);for(let c of A){let _=I(c.name);if(_){n?.(`Using ${c.name}...`);try{let d=await _.execute(c.args);o?.(c.name,c.args,d);let u=`

[Tool Result - ${c.name}]:
${d}

Now respond to the user based on this information. Do NOT use any more tools \u2014 give your final answer.`;r+=u}catch(d){let u=d instanceof Error?d.message:String(d);console.error(`[Agent] Tool ${c.name} failed:`,u),r+=`

[Tool Error - ${c.name}]: ${u}

The tool failed. Respond to the user explaining the issue.`}}}}return"I apologize, but I wasn't able to complete that task after multiple attempts."}var i=window.oc,f=!1,B=10,C="agent_jina_key";function N(){console.log("\u{1F916} Agent v0.1.0+dev"),console.log("   Build: 2026-06-27 21:00:25"),console.log("   https://github.com/Fahell/agent-perchance")}function U(){try{return localStorage.getItem(C)}catch{return null}}function M(e){try{localStorage.setItem(C,e)}catch(t){console.warn("[Agent] Could not save API key to localStorage:",t)}}function K(){return i?i.thread?typeof i.generateText!="function"?(console.error("\u274C [Agent] oc.generateText not available"),!1):!0:(console.error("\u274C [Agent] oc.thread not available"),!1):(console.error("\u274C [Agent] window.oc not found \u2014 are you running inside Perchance?"),!1)}function H(){document.body.innerHTML=`
  ...
  `,document.body.appendChild(s),document.getElementById("settings-close").addEventListener("click",()=>s.remove()),document.getElementById("settings-save").addEventListener("click",async()=>{let n=document.getElementById("settings-key-input").value.trim(),o=document.getElementById("settings-msg");if(!n){o.textContent="Insira uma chave.",o.style.color="#f87171",o.style.display="block";return}o.textContent="Validando...",o.style.color="#aaa",o.style.display="block",await m(n)?(M(n),g(n),o.textContent="\u2705 Chave salva!",o.style.color="#4ade80",setTimeout(()=>s.remove(),1e3)):(o.textContent="\u274C Chave inv\xE1lida.",o.style.color="#f87171")})}function q(e){return e.trim().startsWith("/agent")}function D(e){let t=e.trim();t==="/agent open"?(i.window.show(),console.log("\u{1FA9F} [Agent] Window opened")):t==="/agent close"&&(i.window.hide(),console.log("\u{1FA9F} [Agent] Window closed"))}function b(e){let t=document.getElementById("agent-output");t&&(t.innerHTML+=e,t.scrollTop=t.scrollHeight)}async function V(e){console.log("\u{1F916} [Agent] Processing:",e.content.slice(0,80)),b(`<div style="margin: 8px 0; padding: 8px; background: #16213e; border-radius: 6px; border-left: 3px solid #00d4ff;">
    <div style="color: #00d4ff; font-weight: bold;">\u{1F4E8} ${e.content.slice(0,80)}</div>
  </div>`);let t=i.thread.messages.filter(n=>(n.author==="user"||n.author==="ai")&&n!==e).slice(-B).map(n=>({role:n.author==="user"?"user":"assistant",content:n.content})),s=await O(i,e.content,t,n=>{console.log("\u{1F916} [Agent]",n)},(n,o,a)=>{let r=o.query||o.url||"",l=a.slice(0,300).replace(/\n/g," ");b(`<div style="margin: 4px 0 4px 12px; padding: 6px; background: #0f3460; border-radius: 4px; border-left: 2px solid #4ade80;">
        <div style="color: #4ade80; font-size: 12px;">\u{1F527} ${n}: ${r}</div>
        <div style="color: #aaa; font-size: 11px; margin-top: 4px;">${l}...</div>
      </div>`)});console.log("\u{1F916} [Agent] Response:",s.slice(0,100)),f=!1,i.thread.messages.push({author:"ai",content:s}),b(`<div style="margin: 4px 0 8px 12px; padding: 6px; background: #1a1a2e; border-radius: 4px; border-left: 2px solid #00d4ff;">
    <div style="color: #00d4ff; font-size: 12px;">\u2705 Response sent to chat (${s.length} chars)</div>
  </div>`)}function w(){F(),i.thread.on("MessageAdded",function({message:e}){if(e.author==="user"&&(e.expectsReply=!1,e.hiddenFrom||(e.hiddenFrom=[]),e.hiddenFrom.includes("ai")||e.hiddenFrom.push("ai"),console.log("\u{1F6E1}\uFE0F [Agent] Set expectsReply=false, hiddenFrom=[ai] on user message")),e.author==="ai"&&f){let t=i.thread.messages.indexOf(e);t!==-1&&(i.thread.messages.splice(t,1),console.log("\u{1F5D1}\uFE0F [Agent] Removed internal generator message"));return}if(e.author==="user"){if(q(e.content)){D(e.content),setTimeout(()=>{let t=i.thread.messages.indexOf(e);t!==-1&&i.thread.messages.splice(t,1)},100);return}console.log("\u{1F4E8} [Agent] Processing:",e.content.slice(0,80)),f=!0,V(e).catch(t=>{f=!1,console.error("\u274C [Agent] Error:",t),i.thread.messages.push({author:"ai",content:`Sorry, I encountered an error: ${t instanceof Error?t.message:String(t)}`})})}}),console.log("\u2705 [Agent] Ready!")}function W(){N(),console.log("\u{1F680} [Agent] Loading...");let e=U();e?(g(e),console.log("\u{1F511} [Agent] API key loaded from storage"),w()):(console.log("\u{1F511} [Agent] No API key found \u2014 showing setup screen"),H())}K()&&W();
