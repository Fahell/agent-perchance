...
${t.content}`}}};function T(e){return g[e]}function v(){return Object.values(g).map(e=>`- ${e.name}: ${e.description}
  Parameters: ${JSON.stringify(e.parameters)}`).join(`
`)}function b(e){return e in g}var $=5,f=/<tool_call\s+name="(\w+)">\s*(\{.*?\})\s*<\/tool_call>/gs;function A(){return`You have access to the following tools:
...
- For general knowledge questions, respond directly without using tools.`}function O(e){let t=[],n;for(f.lastIndex=0;(n=f.exec(e))!==null;){let[,o,r]=n;try{let s=JSON.parse(r);b(o)&&t.push({name:o,args:s})}catch{console.warn(`[Agent] Failed to parse tool_call args: ${r}`)}}return t}function _(e){return e.replace(f,"").trim()}async function R(e,t,n,o){let s=`${A()}

User message: ${t}`,l=0;for(;l<$;){l++,n?.(`Thinking... (step ${l})`);let d=(await e.generateText({instruction:s})).toString(),m=O(d);if(m.length===0)return _(d);for(let c of m){let x=T(c.name);if(x){n?.(`Using ${c.name}...`);try{let u=await x.execute(c.args);o?.(c.name,c.args,u);let p=`
...
The tool failed. Respond to the user explaining the issue.`}}}}return"I apologize, but I wasn't able to complete that task after multiple attempts."}var i=window.oc;function S(){return i?i.thread?typeof i.generateText!="function"?(console.error("\u274C [Agent] oc.generateText not available"),!1):!0:(console.error("\u274C [Agent] oc.thread not available"),!1):(console.error("\u274C [Agent] window.oc not found \u2014 are you running inside Perchance?"),!1)}function E(){document.body.innerHTML=`
  ...
  `,i.window.show(),console.log("\u{1FA9F} [Agent] Window opened")}function k(e){return e.trim().startsWith("/agent")}function L(e){let t=e.trim();t==="/agent open"?(i.window.show(),console.log("\u{1FA9F} [Agent] Window opened")):t==="/agent close"&&(i.window.hide(),console.log("\u{1FA9F} [Agent] Window closed"))}function h(e){let t=document.getElementById("agent-output");t&&(t.innerHTML+=e,t.scrollTop=t.scrollHeight)}async function P(e){console.log("\u{1F916} [Agent] Processing:",e.content.slice(0,80)),h(`<div style="margin: 8px 0; padding: 8px; background: #16213e; border-radius: 6px; border-left: 3px solid #00d4ff;">
  ...
  </div>`)}function j(){console.log("\u{1F680} [Agent] Loading..."),E(),i.thread.on("MessageAdded",function({message:e}){if(e.author==="user"){if(k(e.content)){e.expectsReply=!1,e.hiddenFrom=["ai"],L(e.content),setTimeout(()=>{let t=i.thread.messages.indexOf(e);t!==-1&&i.thread.messages.splice(t,1)},100);return}e.expectsReply=!1,e.hiddenFrom=["ai"],P(e).catch(t=>{console.error("\u274C [Agent] Error:",t),i.thread.messages.push({author:"ai",content:`Sorry, I encountered an error: ${t instanceof Error?t.message:String(t)}`})})}}),console.log("\u2705 [Agent] Ready!")}S()&&j();
