var b="";function h(e){b=e}function A(){return b}async function w(e){try{return(await fetch("https://s.jina.ai/",{method:"POST",headers:{Authorization:`Bearer ${e}`,"Content-Type":"application/json",Accept:"application/json"},body:JSON.stringify({q:"test"})})).ok}catch{return!1}}function S(e){return{Authorization:`Bearer ${b}`,"Content-Type":"application/json",Accept:"application/json",...e}}async function E(e,t=5){let s=await fetch("https://s.jina.ai/",{method:"POST",headers:S({"X-Return-Format":"json"}),body:JSON.stringify({q:e,num:t})});if(!s.ok)throw new Error(`Jina search failed: ${s.status} ${s.statusText}`);let n=await s.json(),o=(n.data||n||[]).slice(0,t).map(r=>({title:r.title||"",url:r.url||"",description:r.description||"",content:r.content||""})),a=o.map((r,u)=>`[${u+1}] ${r.title}
    ${r.url}
    ${r.description}`).join(`

`);return{query:e,results:o,raw:a}}async function R(e,t=5e3){let s=await fetch("https://r.jina.ai/",{method:"POST",headers:S({Accept:"text/markdown"}),body:JSON.stringify({url:e})});if(!s.ok)throw new Error(`Jina scrape failed: ${s.status} ${s.statusText}`);let n=await s.text(),o=n.match(/^#\s+(.+)/m),a=o?o[1]:new URL(e).hostname,r=n.length>t?n.slice(0,t)+`

[...truncated]`:n;return{url:e,title:a,content:r}}var k={web_search:{name:"web_search",description:"Search the web for REAL-TIME or CURRENT information. USE this for: prices, exchange rates, sports results, news, weather, events, recent facts, or anything you are not 100% sure about. Returns up to 5 results with titles, URLs, and descriptions.",parameters:{query:"The search query string. Be specific \u2014 include topic, year, or context when relevant."},execute:async e=>(await E(e.query,5)).raw||"No results found."},scrape_url:{name:"scrape_url",description:"Fetch and extract the full text content from a specific URL as markdown. USE this when you have a URL and need to read its content.",parameters:{url:"The full URL to scrape (must start with http:// or https://)"},execute:async e=>{let t=await R(e.url,5e3);return`# ${t.title}

${t.content}`}}};function I(e){return k[e]}function $(){return Object.values(k).map(e=>`- ${e.name}: ${e.description}
  Parameters: ${JSON.stringify(e.parameters)}`).join(`
`)}function M(e){return e in k}var D=5;var _=/<tool_call\s+name="(\w+)">\s*(\{.*?\})\s*<\/tool_call>/gs;function N(){return`You are a helpful assistant with access to web search.
...
Never make up data \u2014 if the search fails, tell the user.`}function F(e){let t=[],s;for(_.lastIndex=0;(s=_.exec(e))!==null;){let[,n,o]=s;try{let a=JSON.parse(o);M(n)&&t.push({name:n,args:a})}catch{console.warn(`[Agent] Failed to parse tool_call args: ${o}`)}}return t}function U(e){return e.replace(_,"").trim()}async function B(e,t,s=[],n,o){let r=N()+`

`;if(s.length>0){r+=`Recent conversation:
`;for(let g of s){let f=g.role==="user"?"User":"Assistant";r+=`${f}: ${g.content}
`}r+=`
`}r+=`User message: ${t}`;let u=0;for(;u<D;){u++,n?.(`Thinking... (step ${u})`);let f=(await e.generateText({instruction:r})).toString(),x=F(f);if(x.length===0)return U(f);for(let l of x){let y=I(l.name);if(y){n?.(`Using ${l.name}...`);try{let c=await y.execute(l.args);o?.(l.name,l.args,c);let d=`

[Tool Result - ${l.name}]:
${c}

Now respond to the user based on this information. Do NOT use any more tools \u2014 give your final answer.`;r+=d}catch(c){let d=c instanceof Error?c.message:String(c);console.error(`[Agent] Tool ${l.name} failed:`,d),r+=`

[Tool Error - ${l.name}]: ${d}

The tool failed. Respond to the user explaining the issue.`}}}}return"I apologize, but I wasn't able to complete that task after multiple attempts."}var m=null;function O(e){m=e}function H(){return m?.thread?.customData?m.thread.customData:{}}function z(e){try{return H()[e]??void 0}catch(t){console.warn("[Storage] get("+e+") failed:",t);return}}function C(e,t){try{if(!m?.thread?.customData){console.warn("[Storage] set: oc.thread.customData not available");return}m.thread.customData[e]=t}catch(s){console.warn("[Storage] set("+e+") failed:",s)}}var i=window.oc,v=!1,W=10;function J(){console.log("\u{1F916} Agent v0.1.0+83a9769"),console.log("   Build: 2026-06-27 22:35:33"),console.log("   https://github.com/Fahell/agent-perchance")}var P="agent:jina_key";function Y(){return z(P)??null}function L(e){C(P,e)}function q(){return i?i.thread?typeof i.generateText!="function"?(console.error("\u274C [Agent] oc.generateText not available"),!1):!0:(console.error("\u274C [Agent] oc.thread not available"),!1):(console.error("\u274C [Agent] window.oc not found \u2014 are you running inside Perchance?"),!1)}function V(){document.body.innerHTML=`
    <div style="font-family: system-ui; padding: 24px; background: #1a1a2e; color: #eee; height: 100vh; margin: 0; display: flex; align-items: center; justify-content: center;">
      <div style="max-width: 480px; width: 100%;">
        <div style="text-align: center; margin-bottom: 24px;">
          ...
          <span style="font-size: 11px; color: #666;">v0.1.0+83a9769</span>
        </div>
  ...
  `;let e=document.getElementById("api-key-input"),t=document.getElementById("api-key-save"),s=document.getElementById("api-key-skip"),n=document.getElementById("api-key-error"),o=document.getElementById("api-key-success");async function a(){let r=e.value.trim();if(!r){n.textContent="Por favor, insira uma chave de API.",n.style.display="block",o.style.display="none";return}t.textContent="Validando...",t.disabled=!0,n.style.display="none",o.style.display="none",await w(r)?(L(r),h(r),o.textContent="\u2705 Chave v\xE1lida! Iniciando...",o.style.display="block",console.log("\u{1F511} [Agent] API key saved to customData"),setTimeout(()=>T(),800)):(n.textContent="\u274C Chave inv\xE1lida. Verifique e tente novamente.",n.style.display="block",t.textContent="Salvar e Iniciar",t.disabled=!1)}t.addEventListener("click",a),e.addEventListener("keydown",r=>{r.key==="Enter"&&a()}),s.addEventListener("click",()=>{console.log("\u23ED\uFE0F [Agent] Setup skipped (no API key)"),T()}),i.window.show()}function X(){document.body.innerHTML=`
  ...
  `,document.body.appendChild(s),document.getElementById("settings-close").addEventListener("click",()=>s.remove()),document.getElementById("settings-save").addEventListener("click",async()=>{let n=document.getElementById("settings-key-input").value.trim(),o=document.getElementById("settings-msg");if(!n){o.textContent="Insira uma chave.",o.style.color="#f87171",o.style.display="block";return}o.textContent="Validando...",o.style.color="#aaa",o.style.display="block",await w(n)?(L(n),h(n),o.textContent="\u2705 Chave salva!",o.style.color="#4ade80",setTimeout(()=>s.remove(),1e3)):(o.textContent="\u274C Chave inv\xE1lida.",o.style.color="#f87171")})}function Z(e){let t=e.trim();return t.startsWith("/agent")||t.startsWith("/test-storage")}function Q(e){let t=e.trim();t==="/agent open"?(i.window.show(),console.log("\u{1FA9F} [Agent] Window opened")):t==="/agent close"?(i.window.hide(),console.log("\u{1FA9F} [Agent] Window closed")):t==="/test-storage"||t==="/test-storage run"?ee():t==="/test-storage check"?te():t==="/test-storage clean"&&(K(),p(`<div style="margin:8px 0;padding:8px;background:#16213e;border-radius:6px;border-left:3px solid #4ade80;">
  ...
  </div>`);let s=[{label:"1KB",bytes:1024},{label:"5KB",bytes:5*1024},{label:"10KB",bytes:10*1024},{label:"50KB",bytes:50*1024},{label:"100KB",bytes:100*1024},{label:"500KB",bytes:500*1024},{label:"1MB",bytes:1048576},{label:"5MB",bytes:5*1048576},{label:"10MB",bytes:10*1048576},{label:"25MB",bytes:25*1048576},{label:"50MB",bytes:50*1048576},{label:"100MB",bytes:100*1048576}];K();let n=[],o="";for(let{label:a,bytes:r}of s){p(`<div style="margin:2px 0 2px 12px;padding:4px 8px;color:#aaa;font-size:12px;">\u23F3 ${a} (${t(r)})...</div>`);let u="ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789",g="";for(;g.length<r;)g+=u;g=g.slice(0,r);let f=!1,x=0,l=!1,y=0;try{let d=performance.now();i.thread.customData[e+a]=g,x=Math.round(performance.now()-d),f=!0}catch(d){p(`<div style="margin:1px 0 1px 16px;padding:2px 8px;font-size:11px;color:#f87171;">
  ...
  </div>`)}function te(){let e="__test_",t=["1KB","5KB","10KB","50KB","100KB","500KB","1MB","5MB","10MB","25MB","50MB","100MB"],s=i.thread.customData||{},n=0;p(`<div style="margin:8px 0;padding:8px;background:#1a1a2e;border-radius:6px;border-left:3px solid #f59e0b;">
    <div style="color:#f59e0b;font-weight:bold;">\u{1F50D} Verificando persist\xEAncia...</div>
  </div>`);for(let o of t){let a=s[e+o]!==void 0;a&&n++,p(`<div style="margin:1px 0 1px 16px;padding:2px 8px;font-size:11px;color:${a?"#4ade80":"#f87171"};">
      ${a?"\u2705":"\u274C"} ${o}
  ...
  </div>`);let t=i.thread.messages.filter(n=>(n.author==="user"||n.author==="ai")&&n!==e).slice(-W).map(n=>({role:n.author==="user"?"user":"assistant",content:n.content})),s=await B(i,e.content,t,n=>{console.log("\u{1F916} [Agent]",n)},(n,o,a)=>{let r=o.query||o.url||"",u=a.slice(0,300).replace(/\n/g," ");p(`<div style="margin: 4px 0 4px 12px; padding: 6px; background: #0f3460; border-radius: 4px; border-left: 2px solid #4ade80;">
        <div style="color: #4ade80; font-size: 12px;">\u{1F527} ${n}: ${r}</div>
        <div style="color: #aaa; font-size: 11px; margin-top: 4px;">${u}...</div>
      </div>`)});console.log("\u{1F916} [Agent] Response:",s.slice(0,100)),v=!1,i.thread.messages.push({author:"ai",content:s}),p(`<div style="margin: 4px 0 8px 12px; padding: 6px; background: #1a1a2e; border-radius: 4px; border-left: 2px solid #00d4ff;">
    <div style="color: #00d4ff; font-size: 12px;">\u2705 Response sent to chat (${s.length} chars)</div>
  </div>`)}function T(){X(),i.thread.on("MessageAdded",function({message:e}){if(e.author==="user"&&(e.expectsReply=!1,e.hiddenFrom||(e.hiddenFrom=[]),e.hiddenFrom.includes("ai")||e.hiddenFrom.push("ai"),console.log("\u{1F6E1}\uFE0F [Agent] Set expectsReply=false, hiddenFrom=[ai] on user message")),e.author==="ai"&&v){let t=i.thread.messages.indexOf(e);t!==-1&&(i.thread.messages.splice(t,1),console.log("\u{1F5D1}\uFE0F [Agent] Removed internal generator message"));return}if(e.author==="user"){if(Z(e.content)){Q(e.content),setTimeout(()=>{let t=i.thread.messages.indexOf(e);t!==-1&&i.thread.messages.splice(t,1)},100);return}console.log("\u{1F4E8} [Agent] Processing:",e.content.slice(0,80)),v=!0,ne(e).catch(t=>{v=!1,console.error("\u274C [Agent] Error:",t),i.thread.messages.push({author:"ai",content:`Sorry, I encountered an error: ${t instanceof Error?t.message:String(t)}`})})}}),console.log("\u2705 [Agent] Ready!")}async function oe(){J(),console.log("\u{1F680} [Agent] Loading..."),O(i);let e=Y();e?(h(e),console.log("\u{1F511} [Agent] API key loaded from customData"),T()):(console.log("\u{1F511} [Agent] No API key found \u2014 showing setup screen"),V())}q()&&oe();
